...
class MysqlSessionRepository implements SessionRepository {
  ...
  async create(session: AppSession): Promise<void> {
    const query = `
      INSERT INTO sessions (token_hash, user_id, csrf_token_hash, issued_at, expires_at, revoked_at, last_seen_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
  ...
  async findByHashedId(hashedId: string): Promise<AppSession | null> {
    const query = `
      SELECT token_hash, user_id, csrf_token_hash, issued_at, expires_at, revoked_at, last_seen_at
      FROM sessions
  ...
  async updateLastSeen(hashedId: string, lastSeenAt: Date): Promise<void> {
    const query = `UPDATE sessions SET last_seen_at = ? WHERE token_hash = ?`;
    await this.pool.execute(query, [lastSeenAt, hashedId]);
  }

  async revoke(hashedId: string): Promise<void> {
    const query = `UPDATE sessions SET revoked_at = NOW() WHERE token_hash = ?`;
