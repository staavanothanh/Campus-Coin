...
| CI | Workflow có MySQL disposable service. Local typecheck/build/OpenAPI/auth/client-IP tests pass trong evidence bên dưới | [Workflow run #5 trên commit `be7aac6`](https://github.com/staavanothanh/Campus-Coin/actions/runs/36113178742) đã tách riêng suite và chỉ ra hai lỗi trong auth integration; code/test đã được sửa, cần run mới xác nhận |
| Domain owner isolation qua HTTP | Test hai tài khoản ở `test/auth.mysql.integration.test.ts` bao phủ wallet, ledger, savings, category, budget, report và dashboard; request giả `userId` không đổi owner | Run #5 tìm thấy budget của category ngoài owner trả `422` thay vì `404`; đã đổi sang `NOT_FOUND` theo OpenAPI và thêm assertion. Chưa ghi pass trước CI tiếp theo |
