...
| `BLK-MIG-02` | MySQL 8.0.41 fresh migration fail ở `0011` (self-FK không nhận index cùng `ALTER`); đã repair bằng cách tách thứ tự DDL, giữ nguyên objects/semantics | B + Team Leader | Fresh `0001`–`0030` xanh local; checksum `0011` đổi có lý do ghi trong file (chưa shared DB nào apply thành công); chờ CI re-run và Team Leader ack repair |
...
| `BLK-CI-01` | Workflow chia unit, bốn MySQL suite và OpenAPI validation riêng; MySQL service disposable | B + D | Run `36087720681` (code trước repair) vẫn đỏ ở `0011`; cùng nội dung đã xanh local 106/106 + datatest 30/30 sau repair; cần CI re-run trên push mới (chưa thực hiện) |
