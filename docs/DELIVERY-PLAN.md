...
Read-only `npm run db:preflight` đã chạy trên Aiven ngày 2026-09-26: connect/TLS handshake và cipher pass; MySQL `8.4.8`; database tồn tại; `schema_migrations` báo applied=28, pending=0/30 với `0004`/`0005` là external baselines. Charset/collation check và pool-vs-`max_connections` được đánh dấu WARN; giá trị đo được là `utf8mb4/utf8mb4_0900_ai_ci` và pool 5 so với max 76. Không ghi endpoint vào tài liệu. Preflight không xác nhận CA provenance, backup/restore hoặc application smoke. SQL read-only sau apply xác nhận `0028`–`0030`, bảng `db_operation_logs` và hai trigger append-only; trigger có `DEFINER=cc_migrate@%` và `SIGNAL`. `cc_migrate`/`cc_runtime` grants đã được kiểm tra bằng `SHOW GRANTS`; `cc_ops` chưa tồn tại. Điểm còn mở không phải DDL mà là CA provenance/mode, `cc_ops`, runtime smoke bằng đúng `cc_runtime`, restore rehearsal và version-drift test.
...
| `BLK-OPSLOG-01` | Migrations `0028`–`0030` tạo `db_operation_logs` append-only; CLI writer dùng `cc_ops` INSERT-only; CI records stay in GitHub | B | DDL `0028`–`0030` đã apply và đã verify read-only trên Aiven: đủ 3 migration row, bảng, 2 trigger với `DEFINER=cc_migrate@%`; CLI INSERT + UPDATE/DELETE guards đã pass trong full migration chain local (`0001`–`0030` fresh + suite runtime-grants). Còn lại: provision/verify `cc_ops` (INSERT-only), rồi CLI smoke bằng secret được quản lý — operator/secret-manager gated |
...
- [ ] BLK-MIG-02 production leg còn lại: app chuyển `.env` sang `cc_runtime` + smoke/reconcile đúng principal; restore rehearsal (checklist: [`docs/working/restore-rehearsal-checklist.md`](working/restore-rehearsal-checklist.md)); mở PR từ `0.2`/`0.3` để CI chạy 3 gates mới; merge vào `main` khi Team Leader quyết.
