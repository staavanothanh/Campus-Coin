...
| Auth implementation | Email/OTP, Google OIDC, explicit link, opaque session, CSRF/Origin và rate-limit đã có code | Team Leader báo `auth.mysql.integration.test.ts` 8/8 và `test:auth-security` 1/1 pass trên MySQL service có clone, dùng schema test tạm; register/reset staging đã báo thành công; SMTP outage và CSRF/Origin staging còn chờ |
