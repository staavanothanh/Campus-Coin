...
|---|---|---|---|
| HUMAN-A | Developer A | Auth implementation và HTTP tests đã push; Team Leader xác nhận auth staging Phần 2 hoàn tất và báo Google OAuth đã kết nối; CSRF/Origin/logout regression pass CI run #11 trên MySQL cô lập | SMTP/provider failure timeout/retry và CSRF/Origin staging còn chờ evidence; môi trường Google OAuth và việc thử riêng login/connect account chưa nêu |
