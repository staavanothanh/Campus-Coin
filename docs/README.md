...
Thư mục này là nguồn tài liệu chuẩn cho Campus Coin: một ứng dụng Web tiếng Việt giúp sinh viên ghi nhận `income` (thu nhập), `payment` (thanh toán), theo dõi ví, quản lý tiền tiết kiệm và ngân sách. Ứng dụng **không phải** ngân hàng, dịch vụ cho vay, BNPL/pay-later hay dịch vụ tư vấn tài chính.
...
| Chủ đề | Quyết định |
|---|---|
| Ngôn ngữ giao diện/nội dung | Tiếng Việt; thuật ngữ loại giao dịch trong API/domain là đúng hai giá trị `income` và `payment`; nhãn UI lần lượt là “Thu nhập” và “Thanh toán”. |
