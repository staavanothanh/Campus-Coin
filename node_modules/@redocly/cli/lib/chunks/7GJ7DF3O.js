...
`),x=Q(n.path).replace(/[^\\/]+$/,""),P=Object.entries(U).map(([p,_])=>({path:`${x}${p}`,content:`${g}

${_.split(`
`).map(b=>b.startsWith("from ._")?b.replace("from ._","from _"):b).join(`
`).trim()}
`}));return[h,...P]};function ue(e,n){let t=Q(n.outputPath).replace(/^.*[\\/]/,"").replace(/\.py$/,""),s=I(n.model).find(a=>a.op.name===e.name)?.ident??k(e.name,{style:"snake",reserved:N}),r=[...e.pathParams.map(a=>{let c=k(a.name,{style:"snake",reserved:N});return`${c}="<${c}>"`}),...e.queryParams.filter(a=>a.required).map(a=>`${k(a.name,{style:"snake",reserved:N})}=...`),...e.requestBody?["body=..."]:[]];return{lang:"python",label:"Python SDK",source:`from ${t} import Client

client = Client()
result = client.${s}(${r.join(", ")})
`}}var sn=({model:e,output:n,emit:t,pagination:s})=>[{path:n.path.replace(/\.[^.\\/]+$/,".python.md"),content:Z(e,{title:`${e.title} Python SDK reference`,frontmatter:t.docsFrontmatter===!0,language:{name:"python",label:"Python",fence:"python",requires:"The SDK needs `httpx`."},sample:r=>ue(r,{model:e,emit:t,outputPath:n.path}),paginated:new Set(s?.keys()??[])})}];export{y as a,J as b,nn as c,tn as d,ue as e,sn as f};
